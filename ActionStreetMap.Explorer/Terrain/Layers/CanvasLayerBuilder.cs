        ...
        public override void Build(MeshContext context, MeshRegion meshRegion)
        {
            ...
            foreach (var triangle in terrainMesh.Triangles)
            {
                ...
                if (v0.Type == VertexType.FreeVertex)
                    ele0 += Noise.Perlin3D(new Vector3(p0.X, 0, p0.Y), 0.1f);
                ...
                if (v1.Type == VertexType.FreeVertex)
                    ele1 += Noise.Perlin3D(new Vector3(p1.X, 0, p1.Y), 0.1f);
                ...
                if (v2.Type == VertexType.FreeVertex)
                    ele2 += Noise.Perlin3D(new Vector3(p2.X, 0, p2.Y), 0.1f);
                ...
                var firstValue = (Noise.Perlin3D(new Vector3((float)v0.X, ele0, (float)v0.Y), 0.2f) + 1f) / 2f;
