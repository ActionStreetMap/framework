...
namespace ActionStreetMap.Explorer.Terrain.Flat
{
    ...
    internal class TerrainCellBuilder
    {
        ...
        public TerrainCellBuilder(float size, int resolution)
        {
            ...
            _vertexCount = _resolution * _resolution * 6;
            _vertices = new Vector3[_vertexCount];
            _colors = new Color[_vertexCount];
            _triangles = new int[_vertexCount];
        }
        ...
        public void Move(Vector2 position, float[,] heightmap, int xOffset, int yOffset, Gradient defaultGradient)
        {
            _position = position;
        ...
        private void Create(float[,] heightmap, int xOffset, int yOffset, Gradient defaultGradient)
        {
            int index = 0;
            ...
            for (int y = 0; y < _resolution; y++)
            {
                for (int x = 0; x < _resolution; x++)
                {
                    ...
                    // first triangle
                    var firstValue = (Noise.Perlin3D(new Vector3(_position.x + x * stepSize, height00, _position.y + y * stepSize), 0.2f).value + 1f) * 0.5f;
                    ...
                    // second triangle
                    var secondValue = (Noise.Perlin3D(new Vector3(_position.x + (x + 1) * stepSize, height00, _position.y + y * stepSize), 0.2f).value + 1f) * 0.5f;
        ...
        public void Fill(Gradient gradient, int line, int start, int end)
        {
            ...
            for (int i = start; i <= end && i != _resolution; i++)
            {
                ...
                var value = (Noise.Perlin3D(new Vector3(start, line, end), 0.2f).value + 1f) * 0.5f;
