...
namespace ActionStreetMap.Core.Polygons.Tools
{
    /// <summary>
    /// Applies the Cuthill and McKee renumbering algorithm to reduce the bandwidth of the adjacency
    ...
    public class CuthillMcKee
    {
        ...
        /// <summary> Computes the bandwidth of a permuted adjacency matrix. </summary>
        ...
        /// <remarks>
        /// The matrix is defined by the adjacency information and a permutation. The routine also
        /// computes the bandwidth and the size of the envelope.
        /// </remarks>
        ...
        /// <summary> Finds the reverse Cuthill-Mckee ordering for a general graph. </summary>
        ...
        private int[] GenerateRcm()
        {
            ...
            for (i = 0; i < node_num; i++)
            {
                ...
                if (mask[i] != 0)
                {
                    ...
                    // Find a pseudo-peripheral node ROOT. The level structure found by ROOT_FIND is
                    // stored starting at PERM(NUM).
        ...
        /// <summary> RCM renumbers a connected component by the reverse Cuthill McKee algorithm. </summary>
        /// <param name="root">
        /// the node that defines the connected component. It is used as the starting point for the
        /// RCM ordering.
        /// </param>
        /// <param name="mask">
        /// Input/output, int MASK(NODE_NUM), a mask for the nodes. Only those nodes with nonzero
        /// input mask values are considered by the routine. The nodes numbered by RCM will have
        ...
        /// X(1) = ROOT.
        /// 
        /// for ( I = 1 to N-1) Find all unlabeled neighbors of X(I), assign them the next available
        /// labels, in order of increasing degree.
        /// 
        ...
        private void Rcm(int root, int[] mask, int[] perm, int offset, ref int iccsze)
        {
            ...
            // LBEGIN and LVLEND point to the beginning and the end of the current level respectively. 
            while (lvlend < lnbr)
            {
                lbegin = lvlend + 1;
                lvlend = lnbr;
                ...
                for (i = lbegin; i <= lvlend; i++)
                {
                    ...
                    // FNBR and LNBR point to the first and last neighbors of the current node in PERM. 
                    fnbr = lnbr + 1;
                    ...
                    if (lnbr > fnbr)
                    {
                        // Sort the neighbors of NODE in increasing order by degree. Linear
                        // insertion is used.
                        k = fnbr;
        ...
        /// <summary> Finds a pseudo-peripheral node. </summary>
        /// <param name="root">
        /// On input, ROOT is a node in the the component of the graph for which a pseudo-peripheral
        /// node is sought. On output, ROOT is the pseudo-peripheral node obtained.
        /// </param>
        /// <param name="mask">
        /// MASK[NODE_NUM], specifies a section subgraph. Nodes for which MASK is zero are ignored
        /// by FNROOT.
        /// </param>
        ...
        /// Output, int LEVEL_ROW(NODE_NUM+1), the level structure array pair containing the level
        /// structure found.
        /// </param>
        /// <param name="level">
        ...
        /// of the graph.
        /// 
        /// The eccentricity of a node is the maximum distance between that node and any other node
        /// of the graph.
        /// 
        /// A peripheral node is a node whose eccentricity equals the diameter of the graph.
        /// 
        /// A pseudo-peripheral node is an approximation to a peripheral node; it may be a
        /// peripheral node, but all we know is that we tried our best.
        /// 
        /// The routine is given a graph, and seeks pseudo-peripheral nodes, using a modified
        /// version of the scheme of Gibbs, Poole and Stockmeyer. It determines such a node for the
        /// section subgraph specified by MASK and ROOT.
        /// 
        /// The routine also determines the level structure associated with the given
        /// pseudo-peripheral node; that is, how far each node is from the pseudo-peripheral node.
        /// The level structure is returned as a list of nodes LS, and pointers to the beginning of
        /// the list of nodes that are at a distance of 0, 1, 2, ..., NODE_NUM-1 from the
        /// pseudo-peripheral node.
        /// 
        /// Reference: Alan George, Joseph Liu, Computer Solution of Large Sparse Positive Definite
        /// Systems, Prentice Hall, 1981.
        /// 
        /// Norman Gibbs, William Poole, Paul Stockmeyer, An Algorithm for Reducing the Bandwidth
        /// and Profile of a Sparse Matrix, SIAM Journal on Numerical Analysis, Volume 13, pages
        /// 236-250, 1976.
        /// 
        /// Norman Gibbs, Algorithm 509: A Hybrid Profile Reduction Algorithm, ACM Transactions on
        /// Mathematical Software, Volume 2, pages 378-387, 1976.
        /// </remarks>
            ...
            int[] level, int offset)
        {
            ...
            // Extreme cases: A complete graph has a level set of only a single level. Every node is
            // equally good (or bad). or A "line graph" 0--0--0--0--0 has every node in its only
            // level. By chance, we've stumbled on the ideal root.
            if (level_num == 1 || level_num == iccsze)
            {
                return;
            }

            // Pick any node from the last level that has minimum degree as the starting point to
            ...
            for (; ; )
            {
                ...
                // In the unlikely case that ROOT is one endpoint of a line graph, we can exit now. 
                if (iccsze <= level_num)
                {
                    break;
                }
        ...
        /// <summary> Generates the connected level structure rooted at a given node. </summary>
        /// <param name="root"> the node at which the level structure is to be rooted. </param>
        /// <param name="mask">
        /// MASK[NODE_NUM]. On input, only nodes with nonzero MASK are to be processed. On output,
        /// those nodes which were included in the level set have MASK set to 1.
        /// </param>
        /// <param name="level_num">
        /// Output, int LEVEL_NUM, the number of levels in the level structure. ROOT is in level 1.
        /// The neighbors of ROOT are in level 2, and so on.
        /// </param>
        ...
        /// 
        /// The root node chosen by the user is assigned level 1, and masked. All (unmasked) nodes
        /// reachable from a node in level 1 are assigned level 2 and masked. The process continues
        /// until there are no unmasked nodes adjacent to any node in the current level. The number
        /// of levels may vary between 2 and NODE_NUM.
        /// 
        /// Reference: Alan George, Joseph Liu, Computer Solution of Large Sparse Positive Definite
        /// Systems, Prentice Hall, 1981.
        /// </remarks>
            ...
            int[] level, int offset)
        {
            ...
            // LBEGIN is the pointer to the beginning of the current level, and LVLEND points to the
            // end of this level.
            for (; ; )
            {
                lbegin = lvlend + 1;
                ...
                // Generate the next level by finding all the masked neighbors of nodes in the
                // current level.
                for (i = lbegin; i <= lvlend; i++)
                {
                ...
                // Compute the current level width (the number of nodes encountered.) If it is
                // positive, generate the next level.
        ...
        /// <summary> Computes the degrees of the nodes in the connected component. </summary>
        ...
        private void Degree(int root, int[] mask, int[] deg, ref int iccsze, int[] ls, int offset)
        {
            ...
            while (lvsize > 0)
            {
                // LBEGIN is the pointer to the beginning of the current level, and LVLEND points to
                // the end of this level.
                lbegin = lvlend + 1;
                lvlend = iccsze;

                // Find the degrees of nodes in the current level, and at the same time, generate
                // the next level.
                for (i = lbegin; i <= lvlend; i++)
                {
                    node = ls[offset + i - 1];
        ...
        /// <summary> Produces the inverse of a given permutation. </summary>
        ...
        /// <example>
        /// Input: N = 5, A = ( 11, 12, 13, 14, 15 ).
        /// 
        /// Output: A = ( 15, 14, 13, 12, 11 ).
        /// </example>
