...
namespace ActionStreetMap.Core.Polygons.Topology
{
    ...
    public class Triangle : ITriangle
    {
        ...
        // The triangle that fills "outer space," called 'dummytri', is pointed to by every triangle
        // and subsegment on a boundary (be it outer or inner) of the triangulation. Also,
        // 'dummytri' points to one of the triangles on the convex hull (until the holes and
        // concavities are carved), making it possible to find a starting triangle for point location.

        // 'dummytri' and 'dummysub' are generally required to fulfill only a few
        // invariants: their vertices must remain NULL and 'dummytri' must always be bonded (at
        // offset zero) to some triangle on the convex hull of the mesh, via a boundary edge.
        // Otherwise, the connections of 'dummytri' and 'dummysub' may change willy-nilly. This
        // makes it possible to avoid writing a good deal of special-case code (in the edge flip,
        // for example) for dealing with the boundary of the mesh, places where no subsegment is
        // present, and so forth. Other entities are frequently bonded to 'dummytri' and 'dummysub'
        // as if they were real mesh entities, with no harm done.

        internal const int EmptyID = -1;

        internal static readonly Triangle Empty;

        /// <summary> Initializes the dummytri (Triangle.Empty). </summary>
        static Triangle()
        {
            ...
            // Initialize the three adjoining triangles to be "outer space." These will eventually
            // be changed by various bonding operations, but their values don't really matter, as
            // long as they can legally be dereferenced.
            ...
            // Initialize the three adjoining subsegments of 'dummytri' to be the omnipresent subsegment. 
            Empty.subsegs[0].seg = Segment.Empty;
            Empty.subsegs[1].seg = Segment.Empty;
            Empty.subsegs[2].seg = Segment.Empty;
        }

        #endregion Static initialization of "Outer Space" triangle
