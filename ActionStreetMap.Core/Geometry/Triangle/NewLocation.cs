...
{
    /// <summary> Find new Steiner point locations. </summary>
    /// <remarks> http: //www.cise.ufl.edu/~ungor/aCute/index.html </remarks>
    internal class NewLocation
    {
            ...
            ref double xi, ref double eta, bool offcenter, Otri badotri)
        {
            ...
            // for debugging and for keeping circumcenter to use later coordinate value of the circumcenter 
            ...
            switch (orientation)
            {
                case 123: 	// assign necessary information
                    /// smallest angle corner: dest largest angle corner: apex 
                ...
                case 132: 	// assign necessary information
                    /// smallest angle corner: dest largest angle corner: org 
                    xShortestEdge = xao; yShortestEdge = yao;
                ...
                case 213: 	// assign necessary information
                    /// smallest angle corner: org largest angle corner: apex 
                ...
                case 231: 	// assign necessary information
                    /// smallest angle corner: org largest angle corner: dest 
                    xShortestEdge = xda; yShortestEdge = yda;
                    xMiddleEdge = xdo; yMiddleEdge = ydo;
                ...
                case 312: 	// assign necessary information
                    /// smallest angle corner: apex largest angle corner: org 
                ...
                default: // TODO: is this safe?
                    /// smallest angle corner: apex largest angle corner: dest 
                    xShortestEdge = xdo; yShortestEdge = ydo;
                    xMiddleEdge = xda; yMiddleEdge = yda;
                    xLongestEdge = xao; yLongestEdge = yao;
            ...
            if (offcenter && (offconstant > 0.0))
            {
                ...
                if (orientation == 213 || orientation == 231)
                {
                    ...
                    // If the off-center is closer to destination than the circumcenter, use the
                    // off-center instead.
                    /// doubleLY BAD CASE /// 
            ...
            if (almostGood == 1)
            {
                ...
                /// RELOCATION (LOCAL SMOOTHING) /// check for possible relocation of one of
                /// triangle's points ///
                relocated = DoSmoothing(delotri, torg, tdest, tapex, ref newloc);
                /// if relocation is possible, delete that vertex and insert a vertex at the new
                /// location ///
                if (relocated > 0)
                {
                    dx = newloc[0] - torg.x;
                ...
                {
                    // calculate radius of the petal according to angle constraint first find the
                    // visible region, PETAL find the center of the circle and radius
                    ...
                    // finding the center first find the middle point of smallest edge 
                    ...
                    yPetalCtr_2 = yMidOfShortestEdge - Math.Sqrt(petalRadius * petalRadius - (shortestEdgeDist / 4)) * (largestAngleCorner.x -
                        middleAngleCorner.x) / Math.Sqrt(shortestEdgeDist);
                    // find the correct circle since there will be two possible circles calculate
                    // the distance to smallest angle corner
                    ...
                    if (!neighborNotFound)
                    {
                        ...
                        // in order to avoid degenerate cases, we need to do a vector based
                        ...
                        // by intersecting bisectors you will end up with the one you want to walk
                        // on then this line and circle should be intersected
                        ...
                        { // there is at least one intersection point
                            // if it is between circumcenter and intersection if it returns 1.0 this
                            // means we have a voronoi vertex within feasible region
                            ...
                            { // there is no voronoi vertex between intersection point and circumcenter
                                ...
                                {
                                    // if it is inside feasible region, then insert v2 apply
                                    // perturbation find the distance between circumcenter and
                                    // intersection point
                                    ...
                                    ax = ax / d;
                                    ay = ay / d;
                                    // now calculate the new intersection point which is perturbated
                                    // towards the circumcenter
                            ...
                            // for acute triangle case, we need to check if it is ok to use either
                            // of them
                    ...
                    /// DO THE SAME THING FOR THE OTHER DIRECTION /// find the third point of the
                    /// neighbor triangle ///
                    ...
                    if (!neighborNotFound)
                    {
                        ...
                        // we need to find correct intersection point, since line intersects circle
                        // twice this direction is always ACUTE
            ...
            ref double xi, ref double eta, bool offcenter, Otri badotri)
        {
            ...
            if (almostGood == 1)
            {
                ...
                {
                    // calculate radius of the petal according to angle constraint first find the
                    // visible region, PETAL find the center of the circle and radius choose minimum
                    // angle as the maximum of quality angle and the minimum angle of the bad triangle
                    ...
                    // calculate the intersection point of the petal and the slab lines first find
                    // the vector distance between xmid and petal center
                    ...
                    // weather it is obtuse/acute the one closer to the minimum angle corner is the
                    // first direction
                    ...
                    if (!neighborNotFound_first)
                    {
                        ...
                        { // there is at least one intersection point
                            ...
                            if (Math.Abs(voronoiOrInter[0] - 1.0) <= EPS)
                            {
                                ...
                                {
                                    // check if we can go further by picking the slab line and petal
                                    // intersection calculate the distance to the smallest angle
                                    // corner check if we create a bad triangle or not
                                ...
                                lengthConst * ((smallestAngleCorner.x - line_inter_x) *
                                    ...
                                    { // slab intersection point is further away
                                        ...
                                        {
                                            // apply perturbation find the distance between
                                            // circumcenter and intersection point
                                            ...
                                            ax = ax / d;
                                            ay = ay / d;
                                            // now calculate the new intersection point which is
                                            // perturbated towards the circumcenter
                                ...
                                else
                                {
                                    ...
                                    {
                                        // we are not creating a bad triangle neighbor's
                                        // circumcenter is suggested
                                        dxFirstSuggestion = voronoiOrInter[2] - torg.x;
                                        dyFirstSuggestion = voronoiOrInter[3] - torg.y;
                                    }
                                }
                            ...
                            { // there is no voronoi vertex between intersection point and circumcenter
                                ...
                                {
                                    // check if we can go further by picking the slab line and petal
                                    // intersection calculate the distance to the smallest angle corner
                                    if (((smallestAngleCorner.x - petal_slab_inter_x_first) * (smallestAngleCorner.x - petal_slab_inter_x_first) +
                                (smallestAngleCorner.y - petal_slab_inter_y_first) * (smallestAngleCorner.y - petal_slab_inter_y_first) >
                                lengthConst * ((smallestAngleCorner.x - line_inter_x) *
                                        (smallestAngleCorner.x - line_inter_x) +
                            ...
                            { // there is no voronoi vertex between intersection point and circumcenter
                                ...
                                lengthConst * ((smallestAngleCorner.x - line_inter_x) *
                                    ...
                                    { // slab intersection point is further away							;
                                        ...
                                        {
                                            // we are not creating a bad triangle slab intersection
                                            // is advised
                                            dxSecondSuggestion = line_result[2] - torg.x;
                                            dySecondSuggestion = line_result[3] - torg.y;
                                        }
        ...
        /// <summary> Given square of edge lengths of a triangle, 
        // determine its orientation 
        /// </summary> <param name="aodist"></param> <param name="dadist"></param> <param
        /// name="dodist"></param> <returns>Returns a number indicating an orientation.</returns>
        ...
        /// <summary> Checks if smothing is possible for a given bad triangle. </summary>
        /// <param name="badotri"></param>
        /// <param name="torg"></param>
        /// <param name="tdest"></param>
        /// <param name="tapex"></param>
            ...
            ref double[] newloc)
        {
            ...
            // check if the points in counterclockwise order p1[0] = points_p[0]; p1[1] =
            // points_p[1]; p2[0] = points_p[2]; p2[1] = points_p[3]; p3[0] = points_p[4]; p3[1] =
            // points_p[5]; v1 = (vertex)p1; v2 = (vertex)p2; v3 = (vertex)p3;
            // if(counterclockwise(m,b,v1,v2,v3) < 0){ // reverse the order to ccw for(i = 0; i <
            // numpoints_p/2; i++){ temp[0] = points_p[2*i]; temp[1] = points_p[2*i+1];
            // points_p[2*i] = points_p[2*(numpoints_p-1)-2*i]; points_p[2*i+1] =
            // points_p[2*(numpoints_p-1)+1-2*i]; points_p[2*(numpoints_p-1)-2*i] = temp[0];
            // points_p[2*(numpoints_p-1)+1-2*i] = temp[1]; } } m.counterclockcount--; INTERSECTION
            // OF PETALS first check whether the star angles are appropriate for relocation
            ...
            // // check if the points in counterclockwise order v1[0] = points_q[0]; v1[1] =
            // points_q[1]; v2[0] = points_q[2]; v2[1] = points_q[3]; v3[0] = points_q[4]; v3[1] =
            // points_q[5]; if(counterclockwise(m,b,v1,v2,v3) < 0){ // reverse the order to ccw
            // for(i = 0; i < numpoints_q/2; i++){ temp[0] = points_q[2*i]; temp[1] =
            // points_q[2*i+1]; points_q[2*i] = points_q[2*(numpoints_q-1)-2*i]; points_q[2*i+1] =
            // points_q[2*(numpoints_q-1)+1-2*i]; points_q[2*(numpoints_q-1)-2*i] = temp[0];
            // points_q[2*(numpoints_q-1)+1-2*i] = temp[1]; } } m.counterclockcount--; INTERSECTION
            // OF PETALS first check whether the star angles are appropriate for relocation
            ...
            // check if the points in counterclockwise order v1[0] = points_r[0]; v1[1] =
            // points_r[1]; v2[0] = points_r[2]; v2[1] = points_r[3]; v3[0] = points_r[4]; v3[1] =
            // points_r[5]; if(counterclockwise(m,b,v1,v2,v3) < 0){ // reverse the order to ccw
            // for(i = 0; i < numpoints_r/2; i++){ temp[0] = points_r[2*i]; temp[1] =
            // points_r[2*i+1]; points_r[2*i] = points_r[2*(numpoints_r-1)-2*i]; points_r[2*i+1] =
            // points_r[2*(numpoints_r-1)+1-2*i]; points_r[2*(numpoints_r-1)-2*i] = temp[0];
            // points_r[2*(numpoints_r-1)+1-2*i] = temp[1]; } } m.counterclockcount--; INTERSECTION
            // OF PETALS first check whether the star angles are appropriate for relocation
        ...
        /// <summary> Finds the star of a given point. </summary>
        /// <param name="badotri"></param>
        /// <param name="p"></param>
        /// <param name="q"></param>
        /// <param name="r"></param>
        ...
        /// <summary> Gets a neighbours vertex. </summary>
        /// <param name="badotri"></param>
                        ...
                        ref double[] thirdpoint, ref Otri neighotri)
        {
            ...
            for (badotri.orient = 0; badotri.orient < 3; badotri.orient++)
            {
                ...
                // check if it is the one we are looking for by checking the corners first check if
                // the neighbor is nonexistent, since it can be on the border
                if (neighbor.tri.id != Topology.Triangle.EmptyID)
                {
                    // then check if two wanted corners are also in this triangle take the vertices
                    // of the candidate neighbor
        ...
        /// <summary> Find a new point location by wedge intersection. </summary>
            ...
            double[] points, ref double[] newloc)
        {
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                // printf("PETAL CONSTANT (%.12f, %.12f)\n", b.petalcenterconstant,
                // b.petalradiusconstant ); printf("PETAL DIFFS (%.6f, %.6f, %.4f)\n", x01, y01, d01);
                ...
                // cw rotation matrix [cosX sinX; -sinX cosX] cw rotation about (x,y) [ux*cosX +
                // uy*sinX + x - x*cosX - y*sinX; -ux*sinX + uy*cosX + y + x*sinX - y*cosX] ccw
                // rotation matrix [cosX -sinX; sinX cosX] ccw rotation about (x,y) [ux*cosX -
                // uy*sinX + x - x*cosX + y*sinX; ux*sinX + uy*cosX + y - x*sinX - y*cosX]
            ...
            if (numpoints != 0)
            {
                ...
                {
                    // numBadTriangle = 0; for(j= 0; j < numpoints *2-2; j = j+2){
                    // if(testTriangleAngleArea(m,b,&newloc[0],&newloc[1], &points[j], &points[j+1],
                    // &points[j+2], &points[j+3] )){ numBadTriangle++; } }
                    // if(testTriangleAngleArea(m,b, &newloc[0],&newloc[1], &points[0], &points[1],
                    // &points[numpoints*2-2], &points[numpoints*2-1] )){ numBadTriangle++; }
                    // 
                    // if (numBadTriangle == 0) {
                    // 
                    // return 1; }
        ...
        private bool GetWedgeIntersection(int numpoints, double[] points, ref double[] newloc)
        {
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                if (i == 0)
                {
                    ...
                    // printf("smallest edge (%f,%f) (%f,%f)\n", x0,y0, x1,y1); printf("real INITIAL
                    // POLY [%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
            ...
            if (numpoints != 0)
            {
                ...
                for (j = 0; j < 40; j = j + 4)
                {
                    ...
                    // printf("%d 1 INITIAL POLY [%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
                    // %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
                    // %.12f;]\n",num,
                    // initialConvexPoly[0],initialConvexPoly[1],initialConvexPoly[2],initialConvexPoly[3],initialConvexPoly[4],initialConvexPoly[5],initialConvexPoly[6],initialConvexPoly[7],initialConvexPoly[8],initialConvexPoly[9],initialConvexPoly[10],initialConvexPoly[11],initialConvexPoly[12],initialConvexPoly[13],initialConvexPoly[14],initialConvexPoly[15],initialConvexPoly[16],initialConvexPoly[17],initialConvexPoly[18],initialConvexPoly[19]);
                    // printf("line (%f, %f) (%f, %f)\n",wedges[40*s+j],wedges[40*s+1+j],
                    // wedges[40*s+2+j], wedges[40*s+3+j]);
        ...
        /// <summary> Check polygon for min angle. </summary>
        /// <param name="numpoints"></param>
        /// <param name="points"></param>
        ...
        /// <summary>
        /// Given three coordinates of a polygon, tests to see if it satisfies the minimum angle
        /// condition for relocation.
        /// </summary>
        /// <param name="x1"></param>
        /// <param name="y1"></param>
        /// <param name="x2"></param>
        ...
        /// <summary> Given four points representing two lines, returns the intersection point.
        /// </summary> <param name="x1"></param> <param name="y1"></param> <param name="x2"></param>
        /// <param name="y2"></param> <param name="x3"></param> <param name="y3"></param> <param
        /// name="x4"></param> <param name="y4"></param> <param name="p">The intersection
        /// point.</param> <remarks>
        ...
        {
            // x1,y1 P1 coordinates (point of line 1) x2,y2 P2 coordinates (point of line 1) x3,y3
            // P3 coordinates (point of line 2) x4,y4 P4 coordinates (point of line 2) p[1],p[2]
            // intersection coordinates
            // 
            // This function returns a pointer array which first index indicates weather they
            // intersect on one point or not, followed by coordinate pairs.
        ...
        /// <summary>
        /// Returns the convex polygon which is the intersection of the given convex polygon with
        /// the halfplane on the left side (regarding the directional vector) of the given line.
        /// </summary>
        ...
        /// <param name="y2"></param>
        /// <returns></returns>
        /// <remarks> http: //www.mathematik.uni-ulm.de/stochastik/lehre/ws03_04/rt/Geometry2D.ps </remarks>
        private int HalfPlaneIntersection(int numvertices, ref double[] convexPoly, double x1, double y1, double x2, double y2)
        {
            ...
            else
            {
                for (i = 0; i < numpolys; i++)
                {
                    ...
                    // compute the minimum and maximum of the third coordinate of the cross product 
                    ...
                    // and if it is positive, the polygon polys[i] is on the left side of line 
                    if (z > 0.0)
                    {
                        res = polys[i];
                        intFound = 1;
        ...
        /// <summary>
        /// Splits a convex polygons into one or two polygons through the intersection with the
        /// given line (regarding the directional vector of the given line).
        /// </summary>
        /// <param name="numvertices"></param>
        /// <param name="convexPoly"></param>
        /// <param name="x1"></param>
        ...
        {
            // state = 0: before the first intersection (with the line) state = 1: after the first
            // intersection (with the line) state = 2: after the second intersection (with the line)
                ...
                else
                {
                    ...
                    if (Math.Abs(p[1] - convexPoly[j]) <= compConst && Math.Abs(p[2] - convexPoly[j + 1]) <= compConst)
                    {
                        ...
                        else if (state == 0)
                        {
                            ...
                            poly1[2 * poly1counter - 1] = convexPoly[j];
                            poly1[2 * poly1counter] = convexPoly[j + 1];
                            // test whether the polygon is splitted or the line only touches the polygon 
                            if (i + 4 < 2 * numvertices)
                            {
                                ...
                                // the line only splits the polygon when the previous and next
                                // vertex lie on different sides of the line
                                if (s1 != s2 && s1 != 0 && s2 != 0)
                                {
                                    // debug 
                                    case3111++;
            ...
            // after splitting the state must be 0 or 2 (depending whether the polygon was splitted
            // or not)
            if (state != 0 && state != 2)
            {
                // printf("there is something wrong state: %d\n", state); printf("polygon might not
                // be convex!!\n"); printf("case1: %d\ncase2: %d\ncase3: %d\ncase31: %d case311: %d
                // case3111: %d\ncase32: %d\ncase33: %d\n", case1, case2, case3, case31, case311,
                // case3111, case32, case33); printf("numvertices %d\n=============\n", numvertices);
        ...
        /// <summary>
        /// Determines on which side (relative to the direction) of the given line and the point
        /// lies (regarding the directional vector) of the given line.
        /// </summary>
        /// <param name="x1"></param>
        /// <param name="y1"></param>
        /// <param name="x2"></param>
        ...
        /// <param name="y4"></param>
        /// <param name="p"></param>
        /// <remarks> referenced to: http://local.wasp.uwa.edu.au/~pbourke/geometry/ </remarks>
        private void LineLineSegmentIntersection(
            ...
            double x4, double y4, ref double[] p)
        {
            // x1,y1 P1 coordinates (point of line) x2,y2 P2 coordinates (point of line) x3,y3 P3
            // coordinates (point of line segment) x4,y4 P4 coordinates (point of line segment)
            ...
            else
            {
                ...
                {	// check if it is on the line segment
                    // printf("line (%.20f, %.20f) (%.20f, %.20f) line seg (%.20f, %.20f) (%.20f,
        ...
        /// <summary> Returns the centroid of a given polygon </summary>
        /// <param name="numpoints"></param>
        /// <param name="points"></param>
        ...
        /// <remarks> referenced to: http://local.wasp.uwa.edu.au/~pbourke/geometry/sphereline/ </remarks>
        private void CircleLineIntersection(
            double x1, double y1,
            double x2, double y2,
            double x3, double y3, double r, ref double[] p)
        {
            // x1,y1 P1 coordinates [point of line] x2,y2 P2 coordinates [point of line] x3,y3, r P3
            // coordinates(circle center) and radius [circle] p[1],p[2]; p[3],p[4] intersection coordinates
            // 
            // This function returns a pointer array which first index indicates the number of
            // intersection points, followed by coordinate pairs.
        ...
        /// <summary>
        /// This function returns a pointer array which first index indicates the whether the point
        /// is in between the other points, followed by coordinate pairs.
        /// </summary>
        ...
        {
            // now check whether the point is close to circumcenter than intersection point BETWEEN
            // THE POINTS
        ...
        /// <summary>
        /// Given three coordinates of a triangle, tests a triangle to see if it satisfies the
        /// minimum and/or maximum angle condition.
        /// </summary>
        /// <param name="x1"></param>
        /// <param name="y1"></param>
        /// <param name="x2"></param>
        ...
        /// <summary>
        /// Given the triangulation, and a vertex returns the minimum distance to the vertices of
        /// the triangle where the given vertex located.
        /// </summary>
