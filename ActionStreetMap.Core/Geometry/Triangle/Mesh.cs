...
namespace ActionStreetMap.Core.Geometry.Triangle
{
    ...
    public class Mesh
    {
        ...
        public ICollection<Topology.Triangle> Triangles
        {
        ...
        /// <summary> Gets the mesh edges. </summary>
        ...
        /// <summary> Renumber vertex and triangle id's. </summary>
        ...
        /*
        /// <summary> Load a mesh from file (.node/poly and .ele). </summary>
        ...
        /// <summary> Reconstructs a mesh from raw input data. </summary>
        public void Load(InputGeometry input, List<ITriangle> triangles)
        {
            ...
            if (!behavior.Poly)
            {
                // Be careful not to allocate space for element area constraints that will never be
                // assigned any value (other than the default -1.0).
                behavior.VarArea = false;

                // Be careful not to add an extra attribute to each element unless the input
                // supports it (PSLG in, but not refining a preexisting mesh).
        ...
        */
        ...
        /// <summary> Reset all the mesh data. This method will also wipe out all mesh data. </summary>
        ...
        /// <summary>
        /// Construct a mapping from vertices to triangles to improve the speed of point location
        /// for segment insertion.
        /// </summary>
        /// <remarks>
        /// Traverses all the triangles, and provides each corner of each triangle with a pointer to
        /// that triangle. Of course, pointers will be overwritten by other pointers because
        /// (almost) each vertex is a corner of several triangles, but in the end every vertex will
        /// point to some triangle that contains it.
        /// </remarks>
        ...
        {
            Topology.Triangle tri = new Topology.Triangle();
        ...
        /// <summary>
        /// Insert a vertex into a Delaunay triangulation, performing flips as necessary to maintain
        ...
        /// <returns>
        /// If a duplicate vertex or violated segment does not prevent the vertex from being
        /// inserted, the return value will be ENCROACHINGVERTEX if the vertex encroaches upon a
        /// subsegment (and checking is enabled), or SUCCESSFULVERTEX otherwise. In either case,
        /// 'searchtri' is set to a handle whose origin is the newly inserted vertex.
        /// </returns>
        /// <remarks>
        /// The point 'newvertex' is located. If 'searchtri.triangle' is not NULL, the search for
        /// the containing triangle begins from 'searchtri'. If 'searchtri.triangle' is NULL, a full
        /// point location procedure is called. If 'insertvertex' is found inside a triangle, the
        /// triangle is split into three; if 'insertvertex' lies on an edge, the edge is split in
        /// two, thereby splitting the two adjacent triangles into four. Edge flips are used to
        /// restore the Delaunay property. If 'insertvertex' lies on an existing vertex, no action
        /// is taken, and the value DUPLICATEVERTEX is returned. On return, 'searchtri' is set to a
        /// handle whose origin is the existing vertex.
        /// 
        /// InsertVertex() does not use flip() for reasons of speed; some information can be reused
        /// from edge flip to edge flip, like the locations of subsegments.
        /// 
        /// Param 'splitseg': Normally, the parameter 'splitseg' is set to NULL, implying that no
        /// subsegment should be split. In this case, if 'insertvertex' is found to lie on a
        /// segment, no action is taken, and the value VIOLATINGVERTEX is returned. On return,
        /// 'searchtri' is set to a handle whose primary edge is the violated subsegment. If the
        /// calling routine wishes to split a subsegment by inserting a vertex in it, the parameter
        /// 'splitseg' should be that subsegment. In this case, 'searchtri' MUST be the triangle
        /// handle reached by pivoting from that subsegment; no point location is done.
        /// 
        /// Param 'segmentflaws': Flags that indicate whether or not there should be checks for the
        /// creation of encroached subsegments. If a newly inserted vertex encroaches upon
        /// subsegments, these subsegments are added to the list of subsegments to be split if
        /// 'segmentflaws' is set.
        /// 
        /// Param 'triflaws': Flags that indicate whether or not there should be checks for the
        /// creation of bad quality triangles. If bad triangles are created, these are added to the
        /// queue if 'triflaws' is set.
        /// </remarks>
            ...
            ref Osub splitseg, bool segmentflaws, bool triflaws)
        {
            ...
            {
                // Find the location of the vertex to be inserted. Check if a good starting triangle
                // has already been provided by the caller.
                if (searchtri.tri.id == Topology.Triangle.EmptyID)
                {
                    // Find a boundary triangle. 
                    horiz.tri = Topology.Triangle.Empty;
                    horiz.orient = 0;
                    horiz.Sym();
            ...
            {
                // There's already a vertex there. Return in 'searchtri' a triangle whose origin is
                // the existing vertex.
            ...
            if ((intersect == LocateResult.OnEdge) || (intersect == LocateResult.Outside))
            {
                    ...
                    if (brokensubseg.seg != Segment.Empty)
                    {
                        ...
                        if (segmentflaws)
                        {
                            ...
                            if (enq && (behavior.NoBisect == 1))
                            {
                                ...
                                enq = testtri.tri.id != Topology.Triangle.EmptyID;
                            }
                            if (enq)
                            {
                        ...
                        // Return a handle whose primary edge contains the vertex, which has not
                        // been inserted.
                        horiz.Copy(ref searchtri);
                        locator.Update(ref horiz);
                ...
                // Insert the vertex on an edge, dividing one triangle into two (if the edge lies on
                // a boundary) or two triangles into four.
                horiz.Lprev(ref botright);
                botright.Sym(ref botrcasing);
                horiz.Sym(ref topright);
                // Is there a second triangle? (Or does this edge lie on a boundary?) 
                mirrorflag = topright.tri.id != Topology.Triangle.EmptyID;
                if (mirrorflag)
                {
                    topright.Lnext();
            ...
            // Circle around the newly inserted vertex, checking each edge opposite it for the
            // Delaunay property. Non-Delaunay edges are flipped. 'horiz' is always the edge being
            // checked. 'first' marks where to stop circling.
            first = horiz.Org();
            rightvertex = first;
            leftvertex = horiz.Dest();
            ...
            while (true)
            {
                ...
                if (doflip)
                {
                    ...
                    horiz.Sym(ref top);
                    if (top.tri.id == Topology.Triangle.EmptyID)
                    {
                    ...
                    else
                    {
                        ...
                        farvertex = top.Apex();
                        // In the incremental Delaunay triangulation algorithm, any of 'leftvertex',
                        // 'rightvertex', and 'farvertex' could be vertices of the triangular
                        // bounding box. These vertices must be treated as if they are infinitely
                        // distant, even though their "coordinates" are not.
                        ...
                        {
                            // 'farvertex' is infinitely distant and cannot be inside the
                            // circumcircle of the triangle 'horiz'.
                            doflip = false;
                        }
                        else
                        {
                        ...
                        if (doflip)
                        {
                            ...
                            // quadrilateral (the two triangles adjacent to 'horiz'). Identify the
                            ...
                            if (behavior.VarArea)
                            {
                                ...
                                {
                                    // Take the average of the two triangles' area constraints. This
                                    // prevents small area constraints from migrating a long, long
                                    // way from their original location due to flips.
                            ...
                            // On the next iterations, consider the two edges that were exposed
                            // (this is, are now visible to the newly inserted vertex) by the edge flip.
                            horiz.Lprev();
                ...
                if (!doflip)
                {
                    ...
                    horiz.Lnext();
                    horiz.Sym(ref testtri);
                    // Check for finishing a complete revolution about the new vertex, or falling
                    // outside of the triangulation. The latter will happen when a vertex is
                    // inserted at a boundary.
                    if ((leftvertex == first) || (testtri.tri.id == Topology.Triangle.EmptyID))
                    {
        ...
        internal void InsertSubseg(ref Otri tri, int subsegmark)
        {
            ...
            if (newsubseg.seg == Segment.Empty)
            {
                ...
                // Bond new subsegment to the two triangles it is sandwiched between. Note that the
                // facing triangle 'oppotri' might be equal to 'dummytri' (outer space), but the new
                // subsegment is bonded to it all the same.
        ...
        /// <summary>
        /// Transform two triangles to two different triangles by flipping an edge counterclockwise
        /// within a quadrilateral.
        /// </summary>
        /// <param name="flipedge"> Handle to the edge that will be flipped. </param>
        /// <remarks>
        /// Imagine the original triangles, abc and bad, oriented so that the shared edge ab lies in
        /// a horizontal plane, with the vertex b on the left and the vertex a on the right. The
        /// vertex c lies below the edge, and the vertex d lies above the edge. The 'flipedge'
        /// handle holds the edge ab of triangle abc, and is directed left, from vertex a to vertex b.
        /// 
        /// The triangles abc and bad are deleted and replaced by the triangles cdb and dca. The
        /// triangles that represent abc and bad are NOT deallocated; they are reused for dca and
        /// cdb, respectively. Hence, any handles that may have held the original triangles are
        /// still valid, although not directed as they were before.
        /// 
        /// Upon completion of this routine, the 'flipedge' handle holds the edge dc of triangle
        /// dca, and is directed down, from vertex d to vertex c. (Hence, the two triangles have
        /// rotated counterclockwise.)
        /// 
        /// WARNING: This transformation is geometrically valid only if the quadrilateral adbc is
        /// convex. Furthermore, this transformation is valid only if there is not a subsegment
        /// between the triangles abc and bad. This routine does not check either of these
        /// preconditions, and it is the responsibility of the calling routine to ensure that they
        /// are met. If they are not, the streets shall be filled with wailing and gnashing of teeth.
        /// 
        /// Terminology
        /// 
        /// A "local transformation" replaces a small set of triangles with another set of
        /// triangles. This may or may not involve inserting or deleting a vertex.
        /// 
        /// The term "casing" is used to describe the set of triangles that are attached to the
        /// triangles being transformed, but are not transformed themselves. Think of the casing as
        /// a fixed hollow structure inside which all the action happens. A "casing" is only defined
        /// relative to a single transformation; each occurrence of a transformation will involve a
        /// different casing.
        /// </remarks>
        ...
        /// <summary>
        /// Transform two triangles to two different triangles by flipping an edge clockwise within
        /// a quadrilateral. Reverses the flip() operation so that the data structures representing
        ...
        /// 
        /// Upon completion of this routine, the 'flipedge' handle holds the edge cd of triangle
        /// cdb, and is directed up, from vertex c to vertex d. (Hence, the two triangles have
        /// rotated clockwise.)
        /// </remarks>
        ...
        /// <summary> Find the Delaunay triangulation of a polygon that has a certain "nice" shape.
        /// This includes the polygons that result from deletion of a vertex or insertion of a
        /// segment. </summary> <param name="firstedge">The primary edge of the first
        /// triangle.</param> <param name="lastedge">The primary edge of the last triangle.</param>
        /// <param name="edgecount">The number of sides of the polygon, including its base.</param>
        /// <param name="doflip">A flag, wether to perform the last flip.</param> <param
        /// name="triflaws">A flag that determines whether the new triangles should be tested for
        /// quality, and enqueued if they are bad.</param> <remarks>
        // This is a conceptually difficult routine. The starting assumption is that we have a
        // polygon with n sides. n - 1 of these sides are currently represented as edges in the
        // mesh. One side, called the "base", need not be.
        // 
        // Inside the polygon is a structure I call a "fan", consisting of n - 1 triangles that
        // share a common origin. For each of these triangles, the edge opposite the origin is one
        // of the sides of the polygon. The primary edge of each triangle is the edge directed from
        // the origin to the destination; note that this is not the same edge that is a side of the
        // polygon. 'firstedge' is the primary edge of the first triangle. From there, the triangles
        // follow in counterclockwise order about the polygon, until 'lastedge', the primary edge of
        // the last triangle. 'firstedge' and 'lastedge' are probably connected to other triangles
        // beyond the extremes of the fan, but their identity is not important, as long as the fan
        // remains connected to them.
        // 
        // Imagine the polygon oriented so that its base is at the bottom. This puts 'firstedge' on
        // the far right, and 'lastedge' on the far left. The right vertex of the base is the
        // destination of 'firstedge', and the left vertex of the base is the apex of 'lastedge'.
        // 
        // The challenge now is to find the right sequence of edge flips to transform the fan into a
        // Delaunay triangulation of the polygon. Each edge flip effectively removes one triangle
        // from the fan, committing it to the polygon. The resulting polygon has one fewer edge. If
        // 'doflip' is set, the final flip will be performed, resulting in a fan of one (useless?)
        // triangle. If 'doflip' is not set, the final flip is not performed, resulting in a fan of
        // two triangles, and an unfinished triangular polygon that is not yet filled out with a
        // single triangle. On completion of the routine, 'lastedge' is the last remaining triangle,
        // or the leftmost of the last two.
        // 
        // Although the flips are performed in the order described above, the decisions about what
        // flips to perform are made in precisely the reverse order. The recursive
        // triangulatepolygon() procedure makes a decision, uses up to two recursive calls to
        // triangulate the "subproblems" (polygons with fewer edges), and then performs an edge flip.
        // 
        // The "decision" it makes is which vertex of the polygon should be connected to the base.
        // This decision is made by testing every possible vertex. Once the best vertex is found,
        // the two edges that connect this vertex to the base become the bases for two smaller
        // polygons. These are triangulated recursively. Unfortunately, this approach can take
        // O(n^2) time not only in the worst case, but in many common cases. It's rarely a big deal
        // for vertex deletion, where n is rarely larger than ten, but it could be a big deal for
        // segment insertion, especially if there's a lot of long segments that each cut many
        // triangles. I ought to code a faster algorithm some day.
        /// </remarks> 
        ...
        /// <remarks>
        /// The origin of 'deltri' is deleted. The union of the triangles adjacent to this vertex is
        /// a polygon, for which the Delaunay triangulation is found. Two triangles are removed from
        /// the mesh.
        /// 
        /// Only interior vertices that do not lie on segments or boundaries may be deleted.
        /// </remarks>
        internal void DeleteVertex(ref Otri deltri)
        {
            ...
            if (edgecount > 3)
            {
                // Triangulate the polygon defined by the union of all triangles adjacent to the
                // vertex being deleted. Check the quality of the resulting triangles.
        ...
        /// <remarks>
        /// Walks through the list of transformations (flips and a vertex insertion) in the reverse
        /// of the order in which they were done, and undoes them. The inserted vertex is removed
        /// from the triangulation and deallocated. Two triangles (possibly just one) are also deallocated.
        /// </remarks>
        internal void UndoVertex()
        {
            ...
            // Walk through the list of transformations (flips and a vertex insertion) in the
            // reverse of the order in which they were done, and undo them.
            while (flipstack.Count > 0)
            {
                // Find a triangle involved in the last unreversed transformation. 
                fliptri = flipstack.Pop();

                // We are reversing one of three transformations: a trisection of one triangle into
                // three (by inserting a vertex in the triangle), a bisection of two triangles into
                // four (by inserting a vertex in an
                // edge) , or an edge flip.
                if (flipstack.Count == 0)
                {
                    // Restore a triangle that was split into three triangles, so it is again one triangle. 
                ...
                {
                    // Restore two triangles that were split into four triangles, so they are again
                    // two triangles.
                    ...
                    if (gluetri.tri.id != Topology.Triangle.EmptyID)
                    {
                        gluetri.Lnext();
        ...
        {
            // Mark the triangle as dead. This makes it possible to detect dead triangles when
            // traversing the list of all triangles.
        ...
        {
            // Mark the vertex as dead. This makes it possible to detect dead vertices when
            // traversing the list of all vertices.
        ...
        {
            // Mark the subsegment as dead. This makes it possible to detect dead subsegments when
            // traversing the list of all subsegments.
