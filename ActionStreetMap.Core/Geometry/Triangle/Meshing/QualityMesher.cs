...
namespace ActionStreetMap.Core.Geometry.Triangle.Meshing
{
    ...
    internal class QualityMesher
    {
        ...
        /// <remarks>
        /// A subsegment is encroached if there is a vertex in its diametral lens. For Ruppert's
        /// algorithm (-D switch), the "diametral lens" is the diametral circle. For Chew's
        /// algorithm (default), the diametral lens is just big enough to enclose two isosceles
        /// triangles whose bases are the subsegment. Each of the two isosceles triangles has two
        /// angles equal to 'b.minangle'.
        /// 
        /// Chew's algorithm does not require diametral lenses at all--but they save time. Any
        /// vertex inside a subsegment's diametral lens implies that the triangle adjoining the
        /// subsegment will be too skinny, so it's only a matter of time before the encroaching
        /// vertex is deleted by Chew's algorithm. It's faster to simply not insert the doomed
        /// vertex in the first place, which is why I use diametral lenses with Chew's algorithm.
        /// </remarks>
        public int CheckSeg4Encroach(ref Osub testsubseg)
        {
            ...
            if (neighbortri.tri.id != Topology.Triangle.EmptyID)
            {
                sides++;
                // Find a vertex opposite this subsegment. 
                eapex = neighbortri.Apex();
                // Check whether the apex is in the diametral lens of the subsegment (the diametral
                // circle if 'conformdel' is set). A dot product of two sides of the triangle is
                // used to check whether the angle at the apex is greater than (180 - 2 'minangle')
                // degrees (for lenses; 90 degrees for diametral circles).
            ...
            if (neighbortri.tri.id != Topology.Triangle.EmptyID)
            {
                ...
                eapex = neighbortri.Apex();
                // Check whether the apex is in the diametral lens of the subsegment (or the
                // diametral circle, if 'conformdel' is set).
                dotproduct = (eorg.x - eapex.x) * (edest.x - eapex.x) +
                             (eorg.y - eapex.y) * (edest.y - eapex.y);
                if (dotproduct < 0.0)
                {
            ...
            {
                // Add the subsegment to the list of encroached subsegments. Be sure to get the
                // orientation right.
        ...
        /// <summary> Test a triangle for quality and size. </summary>
        /// <param name="testtri"> Triangle to check. </param>
        /// <remarks>
        /// Tests a triangle to see if it satisfies the minimum angle condition and the maximum area
        /// condition. Triangles that aren't up to spec are added to the bad triangle queue.
        /// </remarks>
        public void TestTriangle(ref Otri testtri)
        {
            ...
            if ((apexlen > orglen) && (apexlen > destlen))
            {
                // The edge opposite the apex is longest. maxedge = apexlen; Find the cosine of the
                // angle at the apex.
                maxangle = (orglen + destlen - apexlen) / (2 * Math.Sqrt(orglen * destlen));
            }
            else if (orglen > destlen)
            {
                // The edge opposite the origin is longest. maxedge = orglen; Find the cosine of the
                // angle at the origin.
                maxangle = (apexlen + destlen - orglen) / (2 * Math.Sqrt(apexlen * destlen));
            }
            else
            {
                // The edge opposite the destination is longest. maxedge = destlen; Find the cosine
                // of the angle at the destination.
            ...
            {
                // Use the rules of Miller, Pav, and Walkington to decide that certain triangles
                // should not be split, even if they have bad angles. A skinny triangle is not split
                // if its shortest edge subtends a small input angle, and both endpoints of the edge
                // lie on a concentric circular shell. For convenience, I make a small adjustment to
                // that rule: I check if the endpoints of the edge both lie in segment interiors,
                // equidistant from the apex where the two segments meet. First, check if both
                // points lie in segment interiors.
                if ((base1.type == VertexType.SegmentVertex) &&
                    (base2.type == VertexType.SegmentVertex))
                {
                    // Check if both points lie in a common segment. If they do, the skinny triangle
                    // is enqueued to be split as usual.
        ...
        /// <summary>
        /// Traverse the entire list of subsegments, and check each to see if it is encroached. If
        ...
        /// <summary> Split all the encroached subsegments. </summary>
        /// <param name="triflaws">
        /// A flag that specifies whether one should take note of new bad triangles that result from
        /// inserting vertices to repair encroached subsegments.
        /// </param>
        /// <remarks>
        /// Each encroached subsegment is repaired by splitting it - inserting a vertex at or near
        /// its midpoint. Newly inserted vertices may encroach upon other subsegments; these are
        /// also repaired.
        /// </remarks>
        private void SplitEncSegs(bool triflaws)
        {
            ...
            while (badsubsegs.Count > 0)
            {
                ...
                // Make sure that this segment is still the same segment it was when it was
                // determined to be encroached. If the segment was enqueued multiple times (because
                // several newly inserted vertices encroached it), it may have already been split.
                if (!Osub.IsDead(currentenc.seg) && (eorg == seg.org) && (edest == seg.dest))
                {
                    // To decide where to split a segment, we need to know if the segment shares an
                    // endpoint with an adjacent segment. The concern is that, if we simply split
                    // every encroached segment in its center, two adjacent segments with a small
                    // angle between them might lead to an infinite loop; each vertex added to split
                    // one segment will encroach upon the other segment, which must then be split
                    // with a vertex that will encroach upon the first segment, and so on forever.
                    // To avoid this, imagine a set of concentric circles, whose radii are powers of
                    // two, about each segment endpoint. These concentric circles determine where
                    // the segment is split. (If both endpoints are shared with adjacent segments,
                    // split the segment in the middle, and apply the concentric circles for later splittings.)
                    ...
                    // If we're using Chew's algorithm (rather than Ruppert's) to define
                    // encroachment, delete free vertices from the subsegment's diametral circle.
                    ...
                    if (testtri.tri.id != Topology.Triangle.EmptyID)
                    {
                        // Is the destination shared with another segment? 
                        testtri.Lnext();
                        testtri.Pivot(ref testsh);
                    ...
                    // Use the concentric circles if exactly one endpoint is shared with another
                    // adjacent segment.
                    if (acuteorg || acutedest)
                    {
                        segmentlength = Math.Sqrt((edest.x - eorg.x) * (edest.x - eorg.x) +
                                             (edest.y - eorg.y) * (edest.y - eorg.y));
                        // Find the power of two that most evenly splits the segment. The worst case
                        // is a 2:1 ratio between subsegment lengths.
                ...
                // Set subsegment's origin to NULL. This makes it possible to detect dead badsubsegs
                // when traversing the list of all badsubsegs.
                seg.org = null;
            }
        }

        /// <summary> Test every triangle in the mesh for quality measures. </summary>
        ...
        /// <summary>
        /// Inserts a vertex at the circumcenter of a triangle. Deletes the newly inserted vertex if
        /// it encroaches upon a segment.
        /// </summary>
        ...
        private void SplitTriangle(BadTriangle badtri)
        {
            ...
            // Make sure that this triangle is still the same triangle it was when it was tested and
            // determined to be of bad quality. Subsequent transformations may have made it a
            // different triangle.
                ...
                (bdest == badtri.dest) && (bapex == badtri.apex))
            {
                ...
                // Using the original (simpler) Steiner point location method for mesh refinement.
                ...
                {
                    // The new vertex must be in the interior, and therefore is a free vertex with a
                    // marker of zero.
                    ...
                    // Ensure that the handle 'badotri' does not represent the longest edge of the
                    // triangle. This ensures that the circumcenter must fall to the left of this
                    // edge, so point location will work. (If the angle org-apex-dest exceeds 90
                    // degrees, then the circumcenter lies outside the org-dest edge, and eta is
                    // negative. Roundoff error might prevent eta from being negative when it should
                    // be, so I test eta against xi.)
                    if (eta < xi)
                    {
                        badotri.Lprev();
                    }

                    // Insert the circumcenter, searching from the edge of the triangle, and
                    ...
                    {   // success == DUPLICATEVERTEX
                        // Couldn't insert the new vertex because a vertex is already there. if
                        // (Log.Verbose) { logger.Warning("New vertex falls on existing vertex.", "Quality.SplitTriangle()");
                        errorflag = true;
                        //}
                    }
                }
        ...
        /// <summary> Remove all the encroached subsegments and bad triangles from the triangulation. </summary>
        public void EnforceQuality()
        {
            ...
            SplitEncSegs(false);
            // At this point, if we haven't run out of Steiner points, the triangulation should be
            // (conforming) Delaunay.
            ...
            if ((behavior.MinAngle > 0.0) || behavior.VarArea || behavior.fixedArea || behavior.UserTest != null)
            {
                ...
                while ((queue.Count > 0) && (mesh.steinerleft != 0))
                {
                    ...
                    if (badsubsegs.Count > 0)
                    {
                        ...
                        // Fix any encroached subsegments that resulted. Record any new bad
                        // triangles that result.
                        SplitEncSegs(true);
                    }
                }
            }

            // At this point, if the "-D" switch was selected and we haven't run out of Steiner
            // points, the triangulation should be (conforming) Delaunay and have no low-quality triangles.
