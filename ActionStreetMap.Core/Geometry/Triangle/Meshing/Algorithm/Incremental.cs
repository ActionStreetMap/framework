...
namespace ActionStreetMap.Core.Geometry.Triangle.Meshing.Algorithm
{
    ...
    public class Incremental : ITriangulator
    {
        ...
        /// <summary> Form a Delaunay triangulation by incrementally inserting vertices. </summary>
        ...
        public Mesh Triangulate(ICollection<Vertex> points)
        {
            ...
            {
                starttri.tri = Topology.Triangle.Empty;
        ...
        /// <summary> Form an "infinite" bounding triangle to insert vertices into. </summary>
        /// <remarks>
        /// The vertices at "infinity" are assigned finite coordinates, which are used by the point
        /// location routines, but (mostly) ignored by the Delaunay edge flip routines.
        /// </remarks>
        private void GetBoundingBox()
        {
            ...
            // Link dummytri to the bounding box so we can always find an edge to begin searching
            // (point location) from.
            Topology.Triangle.Empty.neighbors[0] = inftri;
        }
        ...
        /// <returns> Returns the number of edges on the convex hull of the triangulation. </returns>
        /// <remarks>
        /// The triangular bounding box has three boundary triangles (one for each side of the
        /// bounding box), and a bunch of triangles fanning out from the three bounding box vertices
        /// (one triangle for each edge of the convex hull of the inner mesh). This routine removes
        /// these triangles.
        /// </remarks>
        private int RemoveBox()
        {
            ...
            nextedge.tri = Topology.Triangle.Empty;
            nextedge.orient = 0;
            nextedge.Sym();
            ...
            nextedge.Lnext();
            nextedge.Sym();
            // Find a triangle (on the boundary of the vertex set) that isn't a bounding box triangle. 
            nextedge.Lprev(ref searchedge);
            searchedge.Sym();
            // Check whether nextedge is another boundary triangle adjacent to the first one. 
            nextedge.Lnext(ref checkedge);
            checkedge.Sym();
            if (checkedge.tri.id == Topology.Triangle.EmptyID)
            {
                // Go on to the next triangle. There are only three boundary triangles, and this
                // next triangle cannot be the third one, so it's safe to stop here.
                searchedge.Lprev();
                searchedge.Sym();
            }
            // Find a new boundary edge to search from, as the current search edge lies on a
            // bounding box triangle and will be deleted.
            ...
            while (!nextedge.Equal(finaledge))
            {
                ...
                dissolveedge.Sym();
                // If not using a PSLG, the vertices should be marked now. (If using a PSLG,
                // markhull() will do the job.)
                if (noPoly)
                {
                    // Be careful! One must check for the case where all the input vertices are
                    // collinear, and thus all the triangles are part of the bounding box.
                    // Otherwise, the setvertexmark() call below will cause a bad pointer reference.
                ...
                if (nextedge.tri.id == Topology.Triangle.EmptyID)
                {
                    // Turn the corner. 
