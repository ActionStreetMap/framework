...
namespace ActionStreetMap.Core.Geometry.Triangle
{
    ...
    {
        /// <summary> Test the mesh for topological consistency. </summary>
        public static bool IsConsistent(Mesh mesh)
        {
            ...
            foreach (var t in mesh.triangles.Values)
            {
                ...
                for (tri.orient = 0; tri.orient < 3; tri.orient++)
                {
                    ...
                    tri.Sym(ref oppotri);
                    if (oppotri.tri.id != Topology.Triangle.EmptyID)
                    {
                        ...
                        // Check that both triangles agree on the identities of their shared vertices. 
                        oppoorg = oppotri.Org();
        ...
        /// <summary> Check if the mesh is (conforming) Delaunay. </summary>
        public static bool IsDelaunay(Mesh mesh)
        {
            return IsDelaunay(mesh, false);
        }

        /// <summary> Check if that the mesh is (constrained) Delaunay. </summary>
        public static bool IsConstrainedDelaunay(Mesh mesh)
        {
            return IsDelaunay(mesh, true);
        }

        /// <summary> Ensure that the mesh is (constrained) Delaunay. </summary>
        private static bool IsDelaunay(Mesh mesh, bool constrained)
        {
            ...
            foreach (var tri in mesh.triangles.Values)
            {
                ...
                for (loop.orient = 0; loop.orient < 3; loop.orient++)
                {
                    ...
                    // Only test that the edge is locally Delaunay if there is an adjoining triangle
                    // whose pointer is larger (to ensure that each pair isn't tested twice).
                    shouldbedelaunay = (loop.tri.id < oppotri.tri.id) &&
                           !Otri.IsDead(oppotri.tri) && (oppotri.tri.id != Topology.Triangle.EmptyID) &&
                    ...
                    {
                        // If a subsegment separates the triangles, then the edge is constrained, so
                        // no local Delaunay test should be done.
